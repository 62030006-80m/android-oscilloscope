...
public class Scope extends View
{
    ...
    float myVlot = (float) 1.3;
    float myTime = (float) 0.1;
    private int width;
    private int height;
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh)
    {
        ...
        paint.setColor(Color.argb(255, 215, 204, 199));
...
//        myVlot = (float) 13.0;
        if (xscale < 1.0)
        {
            for (int i = 0; i < xstop - xstart; i += xstep)
            {
                ...
                path.lineTo(x, y/myVlot);
                myPath.lineTo(9000, 0);
            }
        }
        else
        {
...
//                float newMax = voltMax / myVlot;
        ...
        // Green trace edit to black
        paint.setColor(Color.BLUE);
        paint.setAntiAlias(true);
        cb.drawPath(path, paint);
        cb.drawPath(myPath, paint);
